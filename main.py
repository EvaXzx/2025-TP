...
def drawPlayer(app):
    drawLabel('v', app.cx, app.cy, size = 20, rotateAngle = app.angle)
...
def isNotCollision(app, x, y):
    ...
    if 0<=row<app.rows and 0 <= col < app.cols:
        return app.board[row][col] == 'floor' or  app.board[row][col] == None
    return False
...
def onStep(app):
    if app.isRotating:
        if app.key == 'a':
            ...
            app.dirVector = rotateDirVector(app.dirVector, -app.rotationStep)
        elif app.key == 'd':
            app.angle += app.rotationStep
            app.dirVector = rotateDirVector(app.dirVector, app.rotationStep)
    if app.isMoving:
        dx = app.dirVector[0]*app.d if app.key == 'w' else -app.dirVector[0]*app.d if app.key == 's' else 0
        dy = app.dirVector[1]*app.d if app.key == 'w' else -app.dirVector[1]*app.d if app.key == 's' else 0
        newCx = app.cx + dx
        newCy = app.cy + dy
        if isNotCollision(app, newCx, newCy):
            app.cx = newCx
            app.cy = newCy
